...
def get_data(number_phone): 
    ...
    with sync_playwright() as p: 
        ...
        while while_counter2 < 3: 
            name = page.locator('.kSqtzD').text_content(timeout=500)
        ...
        #last seen
        try: 
            last_seen = page.locator('.BHJZrd').text_content(timeout=1000)
        except: 
            last_seen = None
        ...
        #profile
        try: 
            page.locator('[data-sentry-component="SharedMediaTabContentView"] [aria-label="avatar"] img').wait_for(state='attached', timeout=1000)
            page.locator('[data-sentry-component="SharedMediaTabContentView"] [aria-label="avatar"] img').click(force=True)
            
            page.locator('.swiper-slide-active').first.wait_for(state='attached', timeout=1000)
            try:
                profile_img = page.locator('.swiper-slide-active .react-transform-wrapper .react-transform-component img').first
                img_counter = 1
                page.wait_for_selector('.swiper-slide-active .react-transform-wrapper .react-transform-component img', state='visible', timeout=5000)
                ...
                try: 
                    while next_btn.count() > 0 and next_btn.is_visible(): 
                        try: 
                            next_btn.wait_for(state='visible', timeout=1000)
                            next_btn.click(force=True)

                            profile_img = profile_img = page.locator('.swiper-slide-active .react-transform-wrapper .react-transform-component img').first
                            page.wait_for_selector('.swiper-slide-active .react-transform-wrapper .react-transform-component img', state='visible', timeout=5000)
                            profile_img.screenshot(path=f'profile/{number_phone}/bale_{img_counter}.png')
                            img_counter += 1
                        except: 
                            img_counter += 1
                            console.print(f'[red]A Eror In Loading Profile {img_counter} In Bale.[/red]')
                            continue
                        
                except: 
                    console.print('[red]A Problem In Loading Profile(0Bale), Pls Try Again.[/red]')
                    img_counter = None

            except: 
                console.print('[red]A Problem In Loading Profile(1Bale), Pls Try Again.[/red]')
                img_counter = None
        except: 
            img_counter = 0
