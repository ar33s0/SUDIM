...
def get_data(number_phone): 
    ...
    with sync_playwright() as p: 
        ...
        # for caution
        try: 
            page.locator(f'.peer-title:has-text("{number_phone}")').nth(1).wait_for(state='visible', timeout=3000)
            page.locator(f'.peer-title:has-text("{number_phone}")').nth(1).click(force=True)

            page.locator(f'.chat-info:has(.peer-title:has-text("{number_phone}"))').wait_for(state='visible', timeout=3000)
            ...
            while while_counter < 3: 
                try: 
                    ...
                    page.click('.btn-primary.btn-color-primary.rp', force=True)
                    page.locator('.popup-container.z-depth-1').wait_for(state='hidden', timeout=10000)     
        ...
        page.locator('.profile-name').first.wait_for(state='visible', timeout=3000)
        ...
        try: 
            ...
            id_number = None
        #last seen
        last_seen = page.locator('.profile-subtitle').first.text_content(timeout=500)
        #username
        try: 
                ...
                try: 
                    ...
                    while next_btn.count() > 0 and next_btn.is_visible(): 
                            try: 
                                ...
                                console.print(f'[red]A Problem In Loading Profile {img_counter} In Eitaa.\n {e}[/red]')
                except:
                        console.print('[red]A Problem In Loading Profile(0Eitaa), Pls Try Again.[/red]')
                        img_counter = None
            except:
                console.print('[red]A Problem In Loading Profile(1Eitaa), Pls Try Again.[/red]')
                img_counter = None
        except:
            img_counter = 0             
